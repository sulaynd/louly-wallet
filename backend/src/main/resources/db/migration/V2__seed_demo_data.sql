-- Demo/seed data, run once by Flyway right after V1 creates the schema.
-- Note: unlike the old data.sql, this does NOT reset on every app restart — it seeds a fresh
-- database exactly once. To get a clean demo dataset again, reset the database itself
-- (docker compose down -v && docker compose up -d) so Flyway re-runs from V1.
...
-- Per-transfer safety cap. Editable at any time via PUT /api/admin/limits (ROLE_ADMIN).
INSERT INTO transfer_limits (max_amount, updated_at, updated_by) VALUES
(5000.00, now(), 'system-seed');
